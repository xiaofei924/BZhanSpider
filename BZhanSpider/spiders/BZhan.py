...
class BzhanSpider(scrapy.Spider):
    ...
    def parse(self, response):#默认回调
        ...
        # # ranking = response.xpath('').extract_first()
        # play_count = response.xpath('//div[@class="video-data"]/span[contains(@class,"view")]').extract()
        # danmu_count = response.xpath('//div[@class="video-data"]/span[@class="dm"]').extract_first()
        # like_count = response.xpath('/html/body/div[3]/div/div[1]/div[3]/div[1]/span[1]').extract()
        # corn_count = response.xpath('/html/body/div[3]/div/div[1]/div[3]/div[1]/span[2]/text()').extract()
        ...
        # yield scrapy.Request(url=url, callback=self.parse_up_page,dont_filter=True)
    ...
    def parse_up_page(self, response):
        ...
        # return
        item = BzhanspiderItem();
        up_name = response.xpath('//*[@id="h-name"]/text()').extract()
        ...
        print(str(up_name))
        print(up_certification)
        print(up_introduction)
        print(up_focus_count)
        print(up_fans_count)
        print(up_play_count)
